...
export function NavProjects({
  ...
  const threadBeingRenamed = renameThreadId
    ? threads.find((thread) => thread.id === renameThreadId) ?? null
    : null
  ...
  const handleRenameSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!threadBeingRenamed) {
      return
    }

    onRenameThread(threadBeingRenamed.id, renameValue)
    closeRenameDialog()
  }
  ...
  const handleResetAutomaticTitle = () => {
    ...
    onRenameThread(threadBeingRenamed.id, "")
    closeRenameDialog()
  }
  ...
  return (
    <>
      ...
      <AlertDialog open={threadBeingRenamed !== null} onOpenChange={(open) => !open && closeRenameDialog()}>
        <AlertDialogContent size="sm">
          <form onSubmit={handleRenameSubmit} className="space-y-4">
            <AlertDialogHeader>
              <AlertDialogTitle>Rename terminal</AlertDialogTitle>
              <AlertDialogDescription>
            ...
            <Input
              ...
              placeholder={threadBeingRenamed?.lastAutoTitle ?? "Enter a name"}
              autoFocus
              maxLength={80}
            />
            <AlertDialogFooter className="grid grid-cols-1 gap-2 sm:grid-cols-3">
              ...
              <Button type="button" variant="secondary" onClick={handleResetAutomaticTitle} disabled={busy}>
                Auto name
              </Button>
